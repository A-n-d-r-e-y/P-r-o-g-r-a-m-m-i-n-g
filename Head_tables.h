...
Table::Table() {
    cout << "Конструктор Table::Table()" << endl;
    sz = new int;
    color = new char[32];
}

Table::Table(int sz1, const char* color1) {
    cout << endl << "Конструктор Table::Table(int sz1, char *color1)" << endl;
    sz = new int;
    *sz = sz1;
...
Table::Table(int sz1) {
    cout << endl << "Конструктор Table::Table(int sz1)" << endl;
    sz = new int; *sz = sz1;
    color = new char[32];
}
...
Table::~Table() {
    cout << endl << "Деструктор Table::~Table() " << endl;
...
CompTable::CompTable() :Table() {
    cout << endl << "--- Конструктор CompTable::CompTable() :Table() " << endl;
...
CompTable::CompTable(int sz1, const char* color1, int h1, const char* mtrl1)
    :Table(sz1, color1) {
    cout << endl << "--- Конструктор CompTable::CompTable(int sz1, char* color1, int h1, char* mtrl1) :Table(sz1, color1) " << endl;
    h = new int; *h = h1;
...
CompTable::~CompTable() {
    cout << endl << "--- Деструктор CompTable::~CompTable() " << endl;
